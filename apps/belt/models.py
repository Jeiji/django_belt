...
class UserManager(models.Manager):
    ...
    def add_toy(self, toyEntry, userID):
        user = User.objects.filter( id = userID )
        errors = []
        dbToyCheck = Toy.objects.filter( name = toyEntry['name'].lower() )
        if not toyEntry['name'] :
            errors.append('no_entry')
            return ( False , errors )
        if dbToyCheck :
            ToyToEnter = dbToyCheck[0]
        else:
            ToyToEnter = Toy.objects.create( name = toyEntry['name'].lower())
            newToy = Toy.objects.filter( name = toyEntry['name'].lower())
            Maker.objects.create( user = user[0], toy = newToy[0] )
        ToyToEnter.user.add( user[0] )
        print ('%'*400) ,user
        print ('#'*400) ,ToyToEnter

        return (True, toyEntry['name'])
...
class ToyManager(models.Manager):
    def remove(self, userID, toy_id):
        user = User.objects.filter( id = userID )
        Toy.objects.filter( user = user[0] ).delete()
